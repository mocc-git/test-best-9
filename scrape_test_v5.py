#!/usr/bin/env python3
"""GitHub Actions Playwright 抓取脚本 V5（智能 API 轮询等待）

改进：用 wait_for_api() 轮询检查目标 API 是否已返回，替代固定等待时间。
不阻塞其他并发 API 请求（解决 V3 expect_response 失败问题）。
API 返回后立即继续，超时回退到短等待。
"""
import argparse
import datetime
import json
...
def wait_for_api(all_api_data, url_pattern, timeout=15):
    """轮询等待特定 API 响应（不阻塞其他请求）

    检查 all_api_data 中是否已有匹配 url_pattern 的响应。
    如果有，立即返回；如果超时，返回 None。
    """
    start = time.time()
    while time.time() - start < timeout:
        for url in all_api_data:
            if url_pattern in url and all_api_data[url]:
                return all_api_data[url][-1]
        time.sleep(0.3)
    return None


def scrape_one(page, goods_id, item_name=None):
    """抓取单个饰品数据（智能 API 轮询等待版）"""
    print(f"\n{'='*60}", flush=True)
    ...
    try:
        # 1. 访问详情页 + 等待 info/good API
        print(f"  [1] 访问详情页（等待 info/good API）...", flush=True)
        page.goto(detail_url, wait_until="domcontentloaded", timeout=30000)
        # 等待基本信息 API 返回
        info_resp = wait_for_api(all_api_data, API_INFO_GOOD, timeout=15)
        ...
        # 3. 点击 K 线图 + 等待 chartAll API
        print(f"  [3] 点击 K 线图（等待 chartAll API）...", flush=True)
        page.evaluate("""() => {
        ...
        chart_resp = wait_for_api(all_api_data, API_CHART_ALL, timeout=15)
        if chart_resp is None:
            print(f"      chartAll API 超时，回退等待", flush=True)
            page.wait_for_timeout(2000)

        # 4. 切换平台到悠悠有品 + 等待 chartAll API 新响应
        print(f"  [4] 切换平台到悠悠有品（等待 chartAll API）...", flush=True)
        before_chart_count = sum(len(all_api_data[u]) for u in all_api_data if API_CHART_ALL in u)
        ...
        if select_info:
            ...
            # 等待 chartAll API 出现新响应
            waited = wait_for_api(all_api_data, API_CHART_ALL, timeout=15)
            after_chart_count = sum(len(all_api_data[u]) for u in all_api_data if API_CHART_ALL in u)
            if after_chart_count <= before_chart_count:
                # 没有新响应，短等待
                page.wait_for_timeout(2000)
            print(f"      ✓ 切换完成", flush=True)

        # 5. 切换日线 + 等待 chartAll API 新响应
        print(f"  [5] 切换日线（等待 chartAll API）...", flush=True)
        before_chart_count = sum(len(all_api_data[u]) for u in all_api_data if API_CHART_ALL in u)
        page.evaluate("""() => {
        ...
        if canvas_info:
            ...
            for i in range(CHART_SCROLL_TIMES):
                ...
                # 等待 chartAll API 新响应（短 timeout）
                start_wait = time.time()
                while time.time() - start_wait < 8:
                    current_count = len(all_api_data.get(chart_url, [])) if chart_url else 0
                    if current_count > before_resp_count:
                        break
                    time.sleep(0.3)
                else:
                    # 超时，短等待
        ...
        print(f"  [7] 切换 1 小时（等待 chartAll API）...", flush=True)
        before_chart_count = sum(len(all_api_data[u]) for u in all_api_data if API_CHART_ALL in u)
        page.evaluate("""() => {
        ...
        while time.time() - start_wait < 15:
            current_count = sum(len(all_api_data[u]) for u in all_api_data if API_CHART_ALL in u)
        ...
        # 8. 筹码分布 + 等待 chipData API
        print(f"  [8] 点击筹码分布图（等待 chipData API）...", flush=True)
        before_chip_count = sum(len(all_api_data[u]) for u in all_api_data if API_CHIP_DATA in u)
        page.evaluate("""() => {
        ...
        start_wait = time.time()
        while time.time() - start_wait < 15:
            current_count = sum(len(all_api_data[u]) for u in all_api_data if API_CHIP_DATA in u)
            if current_count > before_chip_count:
                break
            time.sleep(0.3)
        else:
            # 超时，重试点击
...
def main():
    parser = argparse.ArgumentParser(description="CSQAQ Playwright 抓取 V5")
    parser.add_argument("--items-json", default="", help="批量 JSON 数组")
    args = parser.parse_args()

    print("=" * 60, flush=True)
    print("  CSQAQ Playwright 抓取 V5（智能 API 轮询等待）", flush=True)
    print("=" * 60, flush=True)
